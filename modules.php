    ...
    <script>
        ...
        (() => {
            ...
            function populateVoices() {
                ...
                // Create a sorted list with indices mapped to original voices array
                const sortedWithIndices = [];
                
                // Add en-US voices with their original indices
                enUSVoices.forEach(voice => {
                    const originalIndex = voices.indexOf(voice);
                    sortedWithIndices.push({ voice, originalIndex, lang: 'en-US' });
                });
                
                // Add other English voices
                otherEnVoices.forEach(voice => {
                    const originalIndex = voices.indexOf(voice);
                    sortedWithIndices.push({ voice, originalIndex, lang: 'en-other' });
                });
                
                // Add other voices
                otherVoices.forEach(voice => {
                    const originalIndex = voices.indexOf(voice);
                    sortedWithIndices.push({ voice, originalIndex, lang: 'other' });
                });
                
                // Keep the "Select Voice" option and add voices below it in sorted order
                voiceSelect.innerHTML = '<option value="" selected>Select Voice</option>';
                
                // Add voices in sorted order (en-US first)
                sortedWithIndices.forEach(({ voice, originalIndex }) => {
                    const option = document.createElement('option');
                    option.value = originalIndex;
                    let displayText = `${voice.name} (${voice.lang})`;
                    if (voice.default) displayText += ' [default]';
                    if (voice.lang.startsWith('en-US')) displayText += ' ✓';
                    option.textContent = displayText;
                    voiceSelect.appendChild(option);
                });
            }
            
            // Android compatibility: voices may not load immediately
            // Try multiple times with delays and also trigger a synthetic event
            ...
            function ensureVoicesLoaded() {
                ...
                if (currentVoices.length > 0) {
                    ...
                    voiceLoadAttempts++;
                    // On Android, sometimes triggering a speak/cancel helps load voices
                    ...
                    setTimeout(ensureVoicesLoaded, 150);
                } else {
                    // After 20 attempts (3 seconds), populate with whatever we have
                    voicesLoaded = true;
                    populateVoices();
                }
            }
            
            // Android sometimes doesn't fire onvoiceschanged, so we also poll
            window.speechSynthesis.onvoiceschanged = ensureVoicesLoaded;
            ensureVoicesLoaded();
            
            // Additional fallback: check periodically on Android
